/* Beacon offline knowledge base — real survival content + full-text search.
   Bilingual (ru/en). Assigns window.BeaconKB. Content is concise, verified
   first-aid / survival guidance. Extend freely or replace with an offline
   Wikipedia (Kiwix ZIM) reader in the native build. */
(function () {
  const CATS = [
    {
    ...
    },
    {
      ...
      articles: [
        { id: 'cold',
          ru: { t: 'Тепло и укрытие', b: ['Изоляция от земли важнее стен — подстилка из веток/картона.', 'Малый объём удерживает тепло лучше большого.', 'Держите одежду сухой; потейте меньше, снимая слои.', 'Голова и шея — основная потеря тепла, укройте.', 'Вход — от ветра; costёр/свеча внутри только с вентиляцией.'] },
